...
// maintenance
String TECH_M = "04112233";        // maintenance technician badge
bool maintenanceMode = false;      // true = cart in maintenance
...
void readRFID() {
    ...
    if (uid == TECH_M) {
        ...
        Serial.print("MAINTENANCE "); Serial.println(maintenanceMode ? "ON" : "OFF");
        lcd.clear();
        lcd.setCursor(0, 0); lcd.print("MAINTENANCE");
        lcd.setCursor(0, 1); lcd.print(maintenanceMode ? "MODE ON" : "MODE OFF");
        logToSD(ts, lastTemp, "-", "Technician", maintenanceMode ? "MAINTENANCE_ON" : "MAINTENANCE_OFF");
        publishMQTT(lastTemp, "-", "Technician", maintenanceMode ? "MAINTENANCE_ON" : "MAINTENANCE_OFF", ts);
        delay(1500);
        rfid.PICC_HaltA();
        rfid.PCD_StopCrypto1();
        return;   // don't fall through to access logic
    }
...
void loop() {
    ...
    if (now - lastReadTime >= READ_INTERVAL) {
        ...
        if (doorJustOpened) {
            ...
            if (doorOpen && !doorWasOpen) {
                doorOpenTime = now;
                doorWasOpen  = true;
                if (!accessPreAuthorized || (millis() - lastAuthTime > AUTH_WINDOW)) {
                    Serial.println(">> INTRUSION: door opened with no valid badge!");
                    currentUser = "INTRUSION";
                    String tsI = getTimestamp();
                    logToSD(tsI, lastTemp, "OPEN", "UNKNOWN", "INTRUSION");
                    publishMQTT(lastTemp, "OPEN", "UNKNOWN", "INTRUSION", tsI);
                    for (int i = 0; i < 3; i++) {
                        digitalWrite(BUZZER_PIN, HIGH); delay(150);
                        digitalWrite(BUZZER_PIN, LOW);  delay(150);
                    }
                } else {
                    Serial.print(">> Cart opened by "); Serial.println(currentUser);
                    accessPreAuthorized = false;   // use up the authorization
                }
            }
            else if (!doorOpen) {
        ...
        else if (currentState == MAINTENANCE)   stateStr = "MAINTENANCE";
    ...
    if (currentState == MAINTENANCE) {
        digitalWrite(BUZZER_PIN, LOW);   // no alarm during maintenance
        buzzState = false;
        // stay awake so the tech can scan again to exit
    }
